# Function to calculate total effects of treatments on mortality


func_results_mort <- function(data,outcome,outcome1, cut_times) {
  
  # Get IPT weights
  ipt_wt_mort(dat_ipweights_mort)
  ...
  data_event_long <- data1 %>% 
    ...
    mutate(
      ...
      # event indicator
      event_36_long = if_else(event_36 == 1, 1, 0),
      compet_36_long = 0
    )
  ...
  # time dependent coefficients for the hazard of mortality 
  ...
  estimates <-  foreach(i = 1:numboot, .packages = c("tidyverse","magrittr","splitstackshape","survival"), .combine = "rbind") %dopar% {
    ...
    source("H:\\My Documents\\GitHub\\apixaban_warfarin_obese\\analysis\\scripts\\09_func_ipcw_mort.R")  
    ...
    # time dependent coefficients for the hazard of death
    data_event_long$drugsub1 <- data_event_long$time_end * data_event_long$drugsub
    data_event_long$drugsub2 <- data_event_long$time_end2 * data_event_long$drugsub
    data_event_long$drugsub3 <- data_event_long$time_end3 * data_event_long$drugsub
  ...
  write.table(results_36, paste0("analysis/output/tables/results_36_ipw_", outcome), sep="\t", quote=FALSE, row.names=FALSE)
