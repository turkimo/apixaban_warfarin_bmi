# Function to calculate total effects of treatments on major bleeding outcomes < 3 years


func_results_safe_bmi_sen <- function(data,outcome,outcome1, cut_times) {
  
  # Get IPT weights
  ipt_wt_safe(dat_ipweights_safe_bmi_sen)
  
  data1 = merge(data,ipt_weights) %>% merge(dat_ipweights_safe_bmi_sen)
  ...
  data_base = merge(data,ipt_weights) %>% merge(dat_ipweights_safe_bmi_sen)
  ...
  write.table(results_36, paste0("analysis/output/tables/results_36_ipw_bmi_sen_", outcome), sep="\t", quote=FALSE, row.names=FALSE)
