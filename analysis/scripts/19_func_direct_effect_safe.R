# Function to calculate direct effects of treatments on major bleeding


func_results_direct_safe <- function(data,outcome,outcome1, cut_times) {
  
  # Get IPT weights
  ipt_wt_safe(dat_ipweights_safe)
  
  data1 = merge(data,ipt_weights) %>% merge(dat_ipweights_safe)
  ...
  # time dependent coefficients for the hazard of major bleeding
  ...
  hazard_treated_ipcw_cens_stab <- nonParametricCumHaz_all(an_data$ipw_sw_all, 
                                                           inputdata = an_data, grp = 1, 
                                                           event_36_long = TRUE)
  
  hazardcomp_treated_ipcw_cens_stab <- nonParametricCumHaz_all(an_data$ipw_sw_all, 
                                                               inputdata = an_data, grp = 1, 
                                                               event_36_long = FALSE)
  ...
  hazardcomp_treated_ipcw_cens_stab_norm <- rep(0, length.out = (length(cut_times)))
  ...
  hazardcomp_treated_ipcw_cens_stab_over <- rep(0, length.out = (length(cut_times)))
  ...
  hazardcomp_treated_ipcw_cens_stab_obese <- rep(0, length.out = (length(cut_times)))
  ...
  # Treatment 0 warf ####
  hazard_untreated_ipcw_cens_stab <- nonParametricCumHaz_all(an_data$ipw_sw_all, 
                                                             inputdata = an_data, grp = 0, 
                                                             event_36_long = TRUE)
  
  hazardcomp_untreated_ipcw_cens_stab <- nonParametricCumHaz_all(an_data$ipw_sw_all, 
                                                                 inputdata = an_data, grp = 0, 
                                                                 event_36_long = FALSE)
  ...
  hazardcomp_untreated_ipcw_cens_stab_norm <- rep(0, length.out = (length(cut_times)))
  ...
  hazardcomp_untreated_ipcw_cens_stab_over <- rep(0, length.out = (length(cut_times)))
  ...
  hazardcomp_untreated_ipcw_cens_stab_obese <- rep(0, length.out = (length(cut_times)))
  ...
  data_base = merge(data,ipt_weights) %>% merge(dat_ipweights_safe)
  ...
  estimates <-  foreach(i = 1:numboot, .packages = c("tidyverse","magrittr","splitstackshape","survival"), .combine = "rbind") %dopar% {
    ...
    source("H:\\My Documents\\GitHub\\apixaban_warfarin_obese\\analysis\\scripts\\10_func_ipcw_safe.R")  
  ...
  write.table(results_36, paste0("analysis/output/tables/results_36_direct_ipw_", outcome), sep="\t", quote=FALSE, row.names=FALSE)
