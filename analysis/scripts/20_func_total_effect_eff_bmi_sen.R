# Function to calculate total effects of treatments on stroke/SE outcomes < 3 years


func_results_eff_bmi_sen <- function(data,outcome,outcome1, cut_times) {
  
  # Get IPT weights
  ipt_wt_eff(dat_ipweights_eff_bmi_sen)
  
  data1 = merge(data,ipt_weights) %>% merge(dat_ipweights_eff_bmi_sen)
  ...
  data_base = merge(data,ipt_weights) %>% merge(dat_ipweights_eff_bmi_sen)
